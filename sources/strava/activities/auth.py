...
async def refresh_strava_token(refresh_token: str) -> Dict[str, Any]:
    """
    Refresh Strava OAuth token using the auth proxy.
    
    Args:
        refresh_token: The refresh token to use
        
    Returns:
        Dict containing new access_token, refresh_token, expires_at, and expires_in
    """
    auth_proxy_url = os.getenv("AUTH_PROXY_URL", "https://auth.jaces.com")
    
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"{auth_proxy_url}/strava/refresh",
                json={
