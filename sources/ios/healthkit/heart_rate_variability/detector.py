"""Heart rate variability transition detector using PELT algorithm."""
...
class HeartRateVariabilityTransitionDetector(BasePELTTransitionDetector):
    """
    PELT-based heart rate variability (HRV) transition detector.
    
    Detects statistically significant changes in heart rate variability using
    ...
    def __init__(
        ...
        gap_threshold_seconds: int = 3600,  # 1 hour (HRV measured less frequently)
        min_segment_size: int = 2,
        penalty_multiplier: float = 1.2,  # Higher to reduce false transitions
        config: Optional[Dict[str, Any]] = None  # Signal configuration
    ):
        """
        Initialize HRV transition detector.
